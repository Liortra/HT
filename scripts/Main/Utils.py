...
def build_files():
    ...
    startTestTimeStamp = dateTimeObj.strftime("%d%m%Y%H%M%S")  # ddMMyyyyHHmmss
    # path to scripts/HTBio_files
    basepath = os.path.dirname(__file__)
    nameFile = startTestTimeStamp + '.HTBio'
    nameVideo = startTestTimeStamp + '.mp4'  # TODO change to mp4 - FOURCC = mp4v
    # Setting for saving the SMI videoWriter
    videoName = os.path.abspath(os.path.join(basepath, "..", "HTBio_files/", nameVideo))
    # videoWriterFourcc = cv2.VideoWriter_fourcc(*'XVID')  # http://www.fourcc.org/codecs.php - list of available codes
    videoWriterFourcc = cv2.VideoWriter_fourcc(*'mp4v')
    videoWriter = cv2.VideoWriter(videoName, videoWriterFourcc, 30.0, (1920, 1080))
    # videoWriter = cv2.VideoWriter(videoName, videoWriterFourcc, 20.0, (640, 480))
